...
def create_app(config_filename=None):
    ...
    if config_filename:
        ...
        app.config.from_mapping(
            ...
            AWS_S3_USERS_BUCKET_NAME=os.getenv('AWS_S3_USERS_BUCKET_NAME', 'default_users_bucket'),
            AWS_DB_USERS_TABLE_NAME=os.getenv('AWS_DB_USERS_TABLE_NAME', 'default_users_table'),
...
def register_routes(app):
    ...
    def pre_register():
        ...
        if exists_username and exists_email:
            app.logger.info(f"Pre-registration failed: Username and email already exist for {email}")
            return jsonify({"error": "User with this username and email already exists"}), 400
        elif exists_username:
            app.logger.info(f"Pre-registration failed: Username already exists for {username}")
            return jsonify({"error": "User with this username already exists"}), 400
        elif exists_email:
            app.logger.info(f"Pre-registration failed: Email already exists for {email}")
    ...
    def verify_email(token):
        ...
        try:
            ...
            email = app.serializer.loads(token, salt='email-confirm-salt', max_age=3600)  # 1 hour validity
            ...
            del app.pending_registrations[email]
            app.logger.info(f"User {email} successfully registered and pending registration removed")
